...
function ComprasSkeleton() {
  return (
    <div className="flex flex-col h-full">
      <Topbar title="Compras" subtitle="Registro de comprobantes de compra" />
...
export default function ComprasPage() {
  const { vouchers, loading, error, refetch } = useVouchers("COMPRA");
  ...
  const filtered = useMemo(() => {
    return vouchers.filter((c) => {
      const matchSearch =
        !search ||
        c.razonSocialEmisor.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const totalCompras = filtered.reduce((sum, c) => sum + (c.total > 0 ? c.total : 0), 0);
  const totalIGV = filtered.reduce((sum, c) => sum + (c.igv || 0), 0);
  const docsConProblemas = filtered.filter((c) => !c.tieneXML || !c.tienePDF || !c.tieneCDR).length;
      ...
      <div className="flex-1 overflow-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            ...
            <p className="text-xl font-bold text-gray-900 mt-1">{formatCurrency(totalCompras)}</p>
            <p className="text-xs text-gray-400 mt-0.5">{filtered.length} comprobantes</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 font-medium">IGV Total</p>
            <p className="text-xl font-bold text-gray-900 mt-1">{formatCurrency(totalIGV)}</p>
            <p className="text-xs text-gray-400 mt-0.5">Crédito fiscal</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          ...
          <div className={`rounded-xl border p-4 shadow-sm ${docsConProblemas > 0 ? "bg-red-50 border-red-200" : "bg-white border-gray-200"}`}>
            <p className="text-xs text-gray-500 font-medium">Con Problemas</p>
            <p className={`text-xl font-bold mt-1 ${docsConProblemas > 0 ? "text-red-600" : "text-gray-900"}`}>
              {docsConProblemas}
            </p>
            <p className="text-xs text-gray-400 mt-0.5">XML/PDF/CDR faltante</p>
          </div>
        </div>
          ...
          <CardContent className="p-4">
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <Input
                  placeholder="Buscar por RUC, razón social, serie..."
                  value={search}
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((comp) => (
                    <tr key={comp.id} className="hover:bg-gray-50 transition-colors group">
                        ...
                        <div className="flex items-center gap-2">
                          <div>
                            ...
                            <Badge variant={
                              ...
                              comp.tipo === "NOTA_CREDITO" ? "warning" :
                              comp.tipo === "BOLETA" ? "secondary" : "outline"
                      ...
                      <td className="py-3 px-4">
                        <p className="text-xs font-medium text-gray-900 max-w-[180px] truncate">{comp.razonSocialEmisor}</p>
                      ...
                      <td className="py-3 px-4 text-center">
                        {comp.afectoDetraccion ? (
                          <div className="flex flex-col items-center">
                            <Badge variant={
                              comp.estadoDetraccion === "PAGADO" ? "success" :
                              comp.estadoDetraccion === "VENCIDO" ? "destructive" : "warning"
                            } className="text-xs">
                      ...
                      <td className="py-3 px-4 text-center">
                        <Badge variant={
                          ...
                          comp.estado === "OBSERVADO" ? "warning" : "secondary"
                        }>
                          {comp.estado}
                        </Badge>
                      </td>
