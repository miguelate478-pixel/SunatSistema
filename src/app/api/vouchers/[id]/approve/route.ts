...
export async function PATCH(
  ...
  try {
    const session = await requireRole(["SUPER_ADMIN", "ADMIN_EMPRESA", "GERENCIA"]);
    ...
    return NextResponse.json({ success: true, data: voucher, message: "Comprobante aprobado exitosamente" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Error al aprobar comprobante";
